...
{
    public virtual List<Expression<Func<T, bool>>> Criteria { get; } = new();
    ...
    protected void ApplySortingList(IEnumerable<string> sorts)
    {
        ...
        var firstSortingItem = enumerable.First();
        var tempSorts = enumerable.Where(s => !s.Contains(firstSortingItem));
    ...
    private void AddInclude(string includeString) => IncludeStrings.Add(includeString);

    private void AddInclude(Expression<Func<T, object>> includeExpression) => Includes.Add(includeExpression);

    private IGridSpecification<T> ApplyFilter(Expression<Func<T, bool>> expr)
    {
        Criteria.Add(expr);
        return this;
    }

    private void ApplySorting(string sort) => this.ApplySorting(sort, nameof(ApplyOrderBy), nameof(ApplyOrderByDescending));

    private void ApplyThenBySorting(string sort) => this.ApplySorting(sort, nameof(ApplyOrderBy), nameof(ApplyThenOrderByDescending));
}