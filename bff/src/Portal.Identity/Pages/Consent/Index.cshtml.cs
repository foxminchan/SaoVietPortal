        ...
        public async Task<IActionResult> OnPost()
        {
            ...
            switch (Input?.Button)
            {
                case "no":
                    ...
                    await _events
                        .RaiseAsync(new ConsentDeniedEvent(User.GetSubjectId(), request.Client.ClientId, request.ValidatedResources.RawScopeValues));
                    break;
            ...
            if (grantedConsent is not null)
            {
                ...
                if (Input is { ReturnUrl: not null })
                {
            ...
            AuthorizationRequest request)
        {
            var vm = new ViewModel
            {
                ...
                IdentityScopes = request.ValidatedResources.Resources.IdentityResources
                    .Select(x => CreateScopeViewModel(x, model?.ScopesConsented is null || model.ScopesConsented?.Contains(x.Name) == true))
                    .ToArray()
            };
